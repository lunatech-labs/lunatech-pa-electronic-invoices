...
│  └──────────────┘  └──────────────┘  └──────────────┘  └────────────┘ │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                 │
│  │Configuration │  │ Multi-tenant │  │ Bus & Audit  │                 │
│  │ pdp-config   │  │  Registry    │  │ pdp-events   │                 │
│  │ YAML/Tenant  │  │  par SIREN   │  │ PG outbox    │                 │
│  └──────────────┘  └──────────────┘  └──────────────┘                 │
└─────────────────────────────────────────────────────────────────────────┘
```

```
                          Bus d'événements (pdp-events)
                  publish ────────────────────────────► fan-out (at-least-once)
                     ▲                                       │
   pipeline ─────────┘   PostgreSQL `events` (outbox)        ├──► WebhooksSubscriber       → AFNOR XP Z12-013
   (LifecycleProcessor)                                      ├──► TraceEventSubscriber     → ES (events idempotents)
                                                             └──► MetricsSubscriber        → Prometheus (à venir)
```
...
| `pdp-trace` | **Archivage** : Elasticsearch (un index par SIREN), XML+PDF via `ExchangeSnapshotProcessor`, `DuplicateCheckProcessor`. Événements consommés depuis `pdp-events` via `TraceEventSubscriber` (idempotent). | 9 |
| `pdp-events` | **Bus d'événements & journal d'audit** : 14 transitions du cycle de vie, outbox PostgreSQL, `DispatcherWorker` at-least-once, isolation par subscriber. Source de vérité pour l'audit. | 9 |
...
**Total : 932 tests** (+ 8 tests ignorés pour génération de fixtures et FOP Java)
...
| [docs/events.md](docs/events.md) | **Bus d'événements interne `pdp-events`** : outbox PostgreSQL, 14 transitions, at-least-once, subscribers, audit |
| [docs/tracabilite.md](docs/tracabilite.md) | Archivage Elasticsearch : architecture, index par SIREN, API (migration vers `pdp-events` en cours) |
