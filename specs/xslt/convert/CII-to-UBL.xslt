...
N.B. The use of UBLExtension will result in warnings when validating the UBL Invoice/CreditNote.-->
	...
	<xsl:template match="/">
		<!-- BR-FR-04 type codes: Invoice=380,389,393,501,386,500,384,471,472,473 CreditNote=261,262,381,396,502,503 -->
		<xsl:variable name="SourceIsInvoice" select="exists(/rsm:CrossIndustryInvoice/rsm:ExchangedDocument[contains('80 82 84 380 383 384 386 389 393 395 471 472 473 500 501 575 623 780', normalize-space(ram:TypeCode))])"/>
		...
		****************************************************************************************************************************************************************************************************
		...
		<xsl:element name="{if ( $SourceIsInvoice ) then 'inv:Invoice' else 'crd:CreditNote'}">
			...
			<cbc:CustomizationID>
				<xsl:value-of select="rsm:CrossIndustryInvoice/rsm:ExchangedDocumentContext/ram:GuidelineSpecifiedDocumentContextParameter/ram:ID"/>
			</cbc:CustomizationID>
			<!-- Process Identifier (BT-23) -->
			<xsl:if test="rsm:CrossIndustryInvoice/rsm:ExchangedDocumentContext/ram:BusinessProcessSpecifiedDocumentContextParameter/ram:ID">
				<cbc:ProfileID>
					<xsl:value-of select="rsm:CrossIndustryInvoice/rsm:ExchangedDocumentContext/ram:BusinessProcessSpecifiedDocumentContextParameter/ram:ID"/>
				</cbc:ProfileID>
			</xsl:if>
			<!-- Invoice Identifier -->
			<cbc:ID>
